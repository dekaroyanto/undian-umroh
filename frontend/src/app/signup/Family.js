  ...
  const handleSubmit = async (values, { setSubmitting, resetForm }) => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:1501/umroh/user/register",
        values
      );
      ...
      if (error.response && error.response.data) {
        ...
        if (code === 1 && type === "error") {
          // Group name already exists error
          const groupName = message.split("'")[1];
          toast.error(`Family name '${groupName}' already exists`);
        } else if (code === 2 && type === "error") {
          // Phone number already registered error
          const phoneNumber = message.split("'")[1];
          toast.error(`Phone number '${phoneNumber}' already exist`);
        } else {
          // Handle other errors
