...
export default function Page() {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      cust_hp: Yup.string().required("Phone Number must be required"),
      password: Yup.string()
    ...
    onSubmit: async (values) => {
      try {
        ...
        const response = await axios.post(
          "http://10.21.9.213:1501/umroh/user/login",
          values,
          {
            headers: {
        ...
        console.error(error);
        setTimeout(() => {
          toastFailed({ title: "No Telp / Password Salah" });
          setLoading(false);
        }, 1000);
      }
    },
        ...
        <div className="hidden md:flex-1 md:basis-[50%] md:flex items-center justify-center px-20">
          <div className="text-white text-start">
            <h1 className="w-full text-3xl font-bold">
              Progam "Umroh Bisikan Hati Ibu"
            </h1>
            <hr className="border-solid border-2 w-[20%] my-4" />
            <p className="text-sm">
              Aplikasi "Umroh Bisikan Hati Ibu" adalah platform inovatif yang
              memberikan kesempatan kepada setiap pelanggan untuk meraih impian
              umroh dengan cara yang menarik dan bermakna. Diselenggarakan
              dengan penuh semangat oleh Transmart, aplikasi ini memungkinkan
              pengguna untuk mengumpulkan poin melalui setiap transaksi, dan
              kemudian memberikan kesempatan untuk memenangkan voucher umroh
              melalui undian yang adil dan transparan.
            </p>
          </div>
        </div>
            ...
            <form onSubmit={formik.handleSubmit}>
              <Input
                ...
                onChange={
                  ...
                  (e) =>
                    formik.setFieldValue("cust_hp", e.target.value.toString()))
                }
